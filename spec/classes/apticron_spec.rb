    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context 'apticron class without any parameters' do
          ...
          it do
            is_expected.to contain_cron('apticron').with(
              ensure: 'present',
              minute: 15,
              hour: '*',
              monthday: '*',
          ...
          it do
            is_expected.to contain_file('/etc/apticron/apticron.conf').with(
              ensure: 'present',
              owner: 'root',
              group: 'root',
              mode: '0644'
          ...
          it { is_expected.to contain_file('/etc/apticron/apticron.conf').with_content(/^EMAIL="root@#{facts[:fqdn]}"$/) }
        end
